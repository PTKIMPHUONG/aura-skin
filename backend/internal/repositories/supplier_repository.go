...
func (repo *supplierRepository) CreateSupplier(supplier models.Supplier) error {
    ...
    supplierMap["is_active"] = true 

    _, err := session.Run(ctx,
        "CREATE (s:Supplier {supplier_id: $supplier_id, supplier_name: $supplier_name, supplier_email: $supplier_email, supplier_phone: $supplier_phone, default_image: $default_image, is_active: $is_active})",
        supplierMap,
    )
    return err
}
...
func (repo *supplierRepository) UpdateSupplier(supplier models.Supplier) error {
    ...
    _, err := session.Run(ctx,
        "MATCH (s:Supplier {supplier_id: $supplier_id}) SET s.supplier_name = $supplier_name, s.supplier_email = $supplier_email, s.supplier_phone = $supplier_phone, s.default_image = $default_image, s.is_active = $is_active",
        supplierMap, 
    )
    return err
}
...
func (repo *supplierRepository) DeleteSupplier(id string) error {
    ...
    _, err := session.Run(ctx, 
        "MATCH (s:Supplier {supplier_id: $id, is_active: true}) SET s.is_active = false RETURN s", 
